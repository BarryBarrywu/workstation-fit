...
test('presents the independent calculator before evidence and related content', async ({ page }) => {
  ...
  await expect(page).toHaveTitle('就位｜找到适合你的桌椅与显示器高度');
  ...
  await expect(page.locator('#episode .platform-links > span')).toHaveCount(4);
  ...
  await expect(bilibiliLink).toHaveAttribute('target', '_blank');
  ...
  const productLink = page.locator('#episode a[href="https://u.jd.com/x1CzbVT"]');
  ...
  await expect(page.locator('#episode').getByRole('link')).toHaveCount(2);
  ...
  const tuttiLink = page.locator('#related-links a[href="https://tutti.barrybarrywu.com/zh/"]');
  await expect(tuttiLink).toContainText('了解 Tutti');
  await expect(tuttiLink.locator('img')).toHaveAttribute('src', '/brand/tutti-icon.png');
